...
//三、获取action参数的值，并做对应的操作
	switch($_GET["action"]){
		case "add": //添加
			...
			//5. 拼装sql语句，并执行添加
			$sql = "insert into shopmsg values(null,'{$name}','{$typeid}',{$price},{$total},'{$pic}','{$note}',{$addtime})";
			//echo $sql;
		...
		case "del": //删除
			...
			$sql = "delete from shopmsg where id={$_GET['id']}";
			mysql_query($sql,$link);
			//执行图片删除
		...
		case "update": //修改
			...
			//5. 执行修改
			$sql = "update shopmsg set name='{$name}',typeid={$typeid},price={$price},total={$total},note='{$note}',pic='{$pic}' where id={$id}";
			//echo $sql;
			mysql_query($sql,$link);
